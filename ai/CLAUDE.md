...
When working on any repository, use the following workflow:

- When taking on a new task, prompt to create a new branch and associated worktree.
  - Default: branch off the main branch (e.g. `main` or `master` depending on the repo), ask user for the branch name.
...
- Name branches with a descriptive prefix based on your username or team convention.
- Keep commits clean:
