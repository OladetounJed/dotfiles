...
Team configuration should be read from `~/dev/work-context/team.json` if available. This file should contain:

- Team name
- Team members (GitHub usernames)
- Current sprint dates
- Support hero rotation

If no work-context is available, ask the user for team details interactively.

## Default Q1 2026 Objectives

Objectives should be read from `~/dev/work-context/goals.json` or `~/dev/work-context/objectives.json` if available.

If no work-context is available, ask the user for current objectives interactively.
...
- Read team configuration from `~/dev/work-context/team.json` if available
- Ask: Who's the support hero this sprint?
- Ask: Is anyone off during the sprint?
...
Read objectives from `~/dev/work-context/goals.json` or ask the user for them.

For each objective, ask about the status:
...
[Link to goals](file:///~/dev/work-context/goals.md)

<!-- Objectives with status emojis will be populated here -->
...
- [PR title as it appears on GitHub](https://github.com/OWNER/repo/pull/123)
- [Another PR title](https://github.com/OWNER/repo/pull/456)

**Another theme:**

- [PR title](https://github.com/OWNER/repo/pull/789)

@username2

**Theme/category name:**

- [PR title](https://github.com/OWNER/repo/pull/111)

## Plan

[Project Board](file:///~/dev/work-context/board.md)
...
> "Let's write your sprint update! First, let me check for work-context configuration..."
>
> Read `~/dev/work-context/team.json` for team members, then ask:
> "Found X team members in work-context. Who's the support hero and is anyone off this sprint?"
...
# Replace OWNER with your GitHub organization (e.g., yourcompany)
gh search prs --author=USERNAME --owner=OWNER --merged --merged-at=">=DATE" --limit=20 --json title,url,closedAt,repository
```
...
- [Exact PR title from GitHub](https://github.com/OWNER/repo/pull/44483)
- [Another exact PR title](https://github.com/OWNER/repo/pull/43334)
```
...
Read the list of repositories from `~/dev/work-context/repos.json` or ask the user which repos to search. Common patterns:
...
Use the `--owner=OWNER` flag with the appropriate GitHub organization.
